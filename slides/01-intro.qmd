...
subtitle: |
  ...
  <span class="deck-meta">Wackerly et al. Ch. 1–7 (parts)<br> 
...
- $(X,Y)$ are [independent]{.alert} iff $\begin{cases} F_{X,Y}(x,y) = F_X(x) F_Y(y) & \text{or equivalently} \\ 
\varrho_{X,Y}(x,y) = \varrho_X(x) \varrho_Y(y) \end{cases}$
...
- [Binomial]{.alert} (free-throw shooting, discrete) $\Bin(n,p)$, $\quad$ $\Bern(p) = \Bin(1,p)$
$$
\varrho(x) : = \Prob(X=x) = \binom{n}{x} p^x (1-p)^{n-x}, \ x =0, \ldots, n; \quad \Ex(X) = np, \quad \var(X) = np(1-p)
$$

- [Exponential]{.alert} (taxi waiting, continuous) $\Exp(\lambda) = \Exp(1/\mu)$
$$
...
- [Multivariate Normal (Gaussian)]{.alert} (limiting distribution, continuous) $\Norm(\mu, \sigma^2)$
$$
...
& = 0 \times p + \int_0^{\infty} x \times (1-p) \lambda \exp(-\lambda x) \, \dif x = (1-p)/ \lambda
\\
...
The **conditional expectation** of a random variable $X$ given an event $B$
(with $\Prob(B)>0$) is the average value of $X$ when we restrict attention to
outcomes where $B$ occurs:
$$
\Ex[X \mid B]
=
\frac{\Ex[X \mathbf{1}_B]}{\Prob(B)}.
$$
...
This is directly analogous to conditional probability, except that we are
averaging **values** instead of **events**.
...
If $(X,Y)$ has a joint density, the conditional expectation of $X$ given $Y=y$
is
$$
\Ex[X \mid Y=y]
=
...
Rather than fixing $y$, we define the conditional expectation given $Y$ as the
random variable
$$
\Ex[X \mid Y]
=
...
Thus, $\Ex[X\mid Y]$ is itself a **random variable** that depends only on the
...
Let $X,Z$ be integrable random variables and let $Y$ be given.

**Linearity**
$$
\Ex[aX+bZ \mid Y]
=
a\,\Ex[X\mid Y] + b\,\Ex[Z\mid Y].
$$

**Taking out what is known**  
If $g(Y)$ is any function of $Y$, then
$$
\Ex[g(Y)X \mid Y]
=
g(Y)\,\Ex[X\mid Y].
...
That is, the overall expectation is a **weighted average** of the expectations
on $A$ and on its complement.

This decomposition is especially useful for bounding expectations by splitting
the sample space into “typical” and “atypical” events.

---

### Law of total expectation {#total-exp}

The **law of total expectation** states that conditioning does not change the
overall mean:
$$
...
Conditional expectation allows us to localize the behavior of a random variable.
By splitting the sample space into suitable events, we can derive probability
bounds such as **Chebyshev’s inequality** by controlling behavior on each event
separately.
...
M_{Z_n}(t) &= \Ex[\exp(tZ_n)] = \Ex[\exp(tY_1/\sqrt{n}) \cdots \exp(tY_n/\sqrt{n})] =  \{\Ex[\exp(tY_1/\sqrt{n})]\}^n \quad \text{by independence} \\
& = [M_Y(t/\sqrt{n})]^n
\end{align*}
